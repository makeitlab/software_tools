...
def pgm2numpy(filename):
    """
    Read a PGM into a list.  Only supports P2 PGM for now.
    """
    fin = None
    debug = True
...
def convert(fname, p, smul=1):
    fbody, ftp = fname.split('.')
    new_fname = fbody + '.png'
    newp = os.path.join(p, new_fname)
...
try:
    ...
    print 'png folder already exists'
